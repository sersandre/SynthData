...
def run_mve_mvs(args, dataset):
    ...
    if dataset.split("_")[-1] != "largeScene":
        ...
        if config["res"] != [1036, 1036]:
            model.disable_resize()
            for file in os.listdir(IMPATH):
                image = torch.tensor(imageio.imread(os.path.join(IMPATH, file)).astype(numpy.float32) / 255.0)
                image = image[None, ...].permute(0, 3, 1, 2)
                rescaled_image = torch.nn.functional.interpolate(image, config["res"], mode = "area")
                rescaled_image = rescaled_image.permute(0, 2, 3, 1)[0, ...]
                outfile = os.path.join(WORKSPACE,"images", os.path.basename(file))
