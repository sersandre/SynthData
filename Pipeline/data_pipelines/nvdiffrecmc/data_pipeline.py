...
####################
# Datapipeline to prepare workspace, load images and masks, generating camera poses,
# rescaling images and setting up training config
####################

# Helper Datatypes for image/mask loading
PILJPEG = "<class, PIL.JpegImagePlugin.JpegImageFile>"
PILIMAGE = "PIL.ImageFile"
PIPELINE = "/home/sersandr/synthetic_data/Pipeline"
HOMEIDR = "/home/sersandr/synthetic_data/nvdiffrecmc"
...
def gen_config(args):
    ...
    print(f"==== generating training config: {NAME} ====")
    json_out = {
    "ref_mesh": REFMESH,
...
if __name__ == "__main__":
    while True:
        ...
        if proceed == "y":
            os.system(f"docker run --gpus all -it --rm -v /home/sersandr/synthetic_data/nvdiffrecmc:/workspace -it nvdiffrec:v1 \
                      python train.py --config configs/{args.config} -o {args.output_path}")
            print("===== finished reconstruction, moving files to pipeline =====")
            shutil.move(os.path.join(HOMEIDR, 'out', CONFIG.rstrip('.json')), os.path.join(PIPELINE, 'out', 'out_NVDIFFRECMC'))
            sys.exit(1)
        else:
            proceed = input(f"rerun COLMAP? [y/n]").lower() # Rerun COLMAP in case COLMAP failes to generate plausible camera poses
            if proceed == "y":
                continue
            sys.exit(1)    
